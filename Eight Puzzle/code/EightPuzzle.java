...
public class EightPuzzle {
    ...
    private void solve() {
        ...
        pq.priorityEnqueue(root);

        while (!pq.isEmpty()) {
            BoardState min = (BoardState) pq.priorityDequeue();
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (newX < 3 && newX >= 0 && newY < 3 && newY >= 0) {
                    ...
                    BoardState child = new BoardState(curBoard, min.getG() + 10, h, min);

                    if(!child.equals(min.getParent()))pq.priorityEnqueue(child);
                }
            }
        }
    }
